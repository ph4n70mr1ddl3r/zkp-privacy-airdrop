...
async fn check_status_via_relayer(
    ...
    let status: CheckStatusResponse = response.json().await.context("Failed to parse response")?;
    ...
    if status.claimed {
        ...
        println!(
            "  {} {}",
            "Recipient:".green(),
            status.recipient.unwrap_or_else(|| "N/A".to_string())
        );
        println!(
            "  {} {}",
            "Transaction:".green(),
            status.tx_hash.unwrap_or_else(|| "N/A".to_string())
        );
...
async fn check_status_on_chain(nullifier: &str, rpc_url: &str) -> Result<()> {
