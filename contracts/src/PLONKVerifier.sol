...
/**
 * @title PlonkVerifier
 * @author 0KIMS association
 * @notice Verifier contract for PLONK zero-knowledge proofs generated by snarkJS
 * @dev Gas cost: ~1.3M gas per verification. This is expensive due to heavy elliptic curve operations.
 * @dev Consider implementing rate limiting or proof batching for production to mitigate DoS risks.
 *
 * IMPORTANT: PROOF FORMAT DOCUMENTATION
 * -------------------------------------
 * This verifier is auto-generated by snarkJS and expects a specific proof format.
 *
 * Current Implementation (PrivacyAirdropPLONK.sol):
 *   - Uses 8-element PLONK proof structure
 *   - Proof elements: A[2], B[2x2], C[2], Z, T1, T2, T3, WXi
 *   - Public inputs: merkle_root, recipient, nullifier
 *   - Total proof elements: 8 field elements
 *
 * Legacy/Alternative Format (this contract accepts 24 elements):
 *   - Auto-generated snarkJS verifier structure
 *   - Accepts up to 24 field elements in calldata
 *   - Only first 8 elements are used for verification
 *
 * The interface IPLONKVerifier specifies 8-element proof format (line 122):
 *   function verifyProof(uint256[8] calldata _proof, ...)
 *
 * NOTE: When integrating with different PLONK proving systems,
 * ensure proofs are generated in the 8-element format expected by
 * PrivacyAirdropPLONK.sol and IPLONKVerifier interface.
 */
